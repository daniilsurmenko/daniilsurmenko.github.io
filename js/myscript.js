...
function sliderHeight() {
	...
	var menu_h = jQuery('.menu_block').height();
	var slider_h = Math.abs(wh - menu_h);
	
	jQuery('.flexslider.top_slider, .flexslider.top_slider .slides .container').css('min-height', slider_h);
...
jQuery(document).ready(function(){
	jQuery('.our-skills').waypoint(function() {
		setTimeout(function() {
			jQuery('.progress-complete').each(function(index) {
				var slidewidth = jQuery(this).attr('data-width');
				jQuery(this).animate({width:slidewidth}, 300);
			});
		}, 1000);
	}, {offset: '100%'});
});

/* __________ Waypoints __________ */
...
$(function() {$( "#slider-range" ).slider({ range: true, min: 0, max: 1500, values: [ 0, 1500 ], slide: function( event, ui ) { $( "#amount" ).val( "$" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] ); } }); $( "#amount" ).val( "$" + $( "#slider-range" ).slider( "values", 0 ) + " - $" + $( "#slider-range" ).slider( "values", 1 ) ); });
